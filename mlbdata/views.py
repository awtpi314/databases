...
def player_details(request, player_id):
    ...
    for ts in player.team_seasons.all():
        if ts.year not in team_seasons:
            team_seasons[ts.year] = [(ts.team.name, ts.team.id)]
        else:
            team_seasons[ts.year].append((ts.team.name, ts.team.id))
...
def season_stats_json(request):
    ...
    top_ten_pitchers = sorted(
        [
            ...
            if pitching_stats[i].outs_pitched > 150
        ],
        key=lambda x: x["era"],
    )[:10]
...
def roster_details(request, team_season_id):
    ...
    player_season = PlayerSeason.objects.filter(
        player__in=roster, year=team_season.year
    )
    ...
    context = {
        "roster": roster,
        "players_season": player_season_lookup,
